...
`while True:
  ...
  fresh:`<ul><li><strong>SDK <code>ResultMessage.subtype</code></strong> — the Agent SDK wraps the loop's termination state in a <code>subtype</code> (<code>success</code>, <code>error_max_turns</code>, <code>error_max_budget_usd</code>, <code>error_during_execution</code>, <code>error_max_structured_output_retries</code>), so your harness reads <em>why</em> the loop ended, not just that it did. <a href="https://code.claude.com/docs/en/agent-sdk/overview">docs</a></li><li><strong>New <code>stop_reason</code> values</strong> — beyond the exam guide's set, current models can return <code>refusal</code> (HTTP 200, with a <code>stop_details</code> policy category) and <code>model_context_window_exceeded</code> (default on Sonnet 4.5+; older models need the <code>model-context-window-exceeded-2025-08-26</code> beta header). <a href="https://platform.claude.com/docs/en/api/handling-stop-reasons">docs</a></li><li><strong><code>pause_turn</code> clarified</strong> — the exam guide treats it as "long-running server tool, resend to continue"; current docs add that it specifically means the server-tool sampling loop hit its iteration cap. <a href="https://platform.claude.com/docs/en/api/handling-stop-reasons">docs</a></li><li><strong><code>effort</code> per-turn</strong> — an <code>effort</code> option (<code>low</code>/<code>medium</code>/<code>high</code>/<code>xhigh</code>/<code>max</code>) now tunes how much reasoning each loop turn spends; <code>xhigh</code> is recommended for Fable 5 / Opus 4.7+. <a href="https://platform.claude.com/docs/en/build-with-claude/extended-thinking">docs</a></li></ul>`,
  quick:[
...
# ALL of the above flows through the coordinator -> one place for errors/logs.` },
  fresh:`<ul><li><strong>Three orchestration tiers now</strong> — the exam guide's coordinator/subagent (one-directional reporting) is one of three: there are also <strong>Agent Teams</strong> (peer-to-peer mailbox messaging plus a shared task list, experimental behind <code>CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS</code>) and background agents / FleetView. <a href="https://code.claude.com/docs/en/agent-sdk/subagents">docs</a></li><li><strong>No "create a team" tool</strong> — the <code>TeamCreate</code>/<code>TeamDelete</code> tools were removed in v2.1.178 and <code>team_name</code> on the Agent tool is now ignored, so explicit team-creation tooling no longer exists; teams form via task assignment instead. <a href="https://code.claude.com/docs/en/agent-sdk/subagents">docs</a></li><li><strong>Auto-unblocking task list</strong> — in the shared task list, task dependencies auto-unblock as predecessors complete, and file locking prevents two agents racing to claim the same work. <a href="https://code.claude.com/docs/en/agent-sdk/subagents">docs</a></li></ul>`,
  quick:[
...
Task(subagent_type="synthesis",
  ...
  fresh:`<ul><li><strong><code>Task</code> tool renamed to <code>Agent</code></strong> — the exam guide's "Task tool / <code>allowedTools</code> must include Task" still describes the concept, but current SDK (v2.1.63+) emits <code>Agent</code> in <code>tool_use</code>; legacy <code>Task</code> still appears in the init tools list. <a href="https://code.claude.com/docs/en/agent-sdk/subagents">docs</a></li><li><strong><code>AgentDefinition</code> gained fields</strong> — beyond description/prompt/tools, it now takes <code>skills</code>, <code>memory</code> (user/project/local), <code>mcpServers</code>, <code>initialPrompt</code>, <code>background</code>, <code>effort</code>, <code>permissionMode</code>, and <code>maxTurns</code>. <a href="https://code.claude.com/docs/en/agent-sdk/subagents">docs</a></li><li><strong>Subagents can spawn subagents</strong> — as of v2.1.172, a subagent can itself spawn subagents (background nesting capped at 5 deep), so the strict two-level hub-and-spoke of the exam guide is no longer the only shape. <a href="https://code.claude.com/docs/en/agent-sdk/subagents">docs</a></li><li><strong><code>disallowedTools</code> takes MCP patterns</strong> — you can now scope a subagent away from whole MCP servers with patterns like <code>mcp__server__*</code>. <a href="https://code.claude.com/docs/en/agent-sdk/subagents">docs</a></li><li><strong>Subagents individually resumable</strong> — capture the <code>agentId</code> from the Agent result and pass <code>resume: &lt;sessionId&gt;</code>; transcripts persist independently (~30 days, <code>cleanupPeriodDays</code>). <a href="https://code.claude.com/docs/en/agent-sdk/subagents">docs</a></li></ul>`,
  quick:[
...
handoff = {
  ...
  fresh:`<ul><li><strong>The <code>Workflow</code> tool</strong> — TS SDK v0.3.149+ adds a <code>Workflow</code> tool that externalizes orchestration of many agents into a script <em>outside</em> the conversation, giving you a deterministic enforcement layer above the prompt. <a href="https://code.claude.com/docs/en/agent-sdk/overview">docs</a></li><li><strong>Quality-gate hooks</strong> — <code>TeammateIdle</code>, <code>TaskCreated</code>, and <code>TaskCompleted</code> hooks act as gates: exit code 2 blocks the action and sends feedback, the same deterministic-block mechanism the exam guide describes for tool gates. <a href="https://code.claude.com/docs/en/hooks">docs</a></li><li><strong><code>PreCompact</code> as a preservation point</strong> — a <code>PreCompact</code> hook (trigger <code>manual</code> or <code>auto</code>) archives transcripts before compaction, and the compactor reads CLAUDE.md summarization instructions — a new programmatic point to preserve handoff state across long workflows. <a href="https://code.claude.com/docs/en/hooks">docs</a></li></ul>`,
  quick:[
...
#   shape before the model sees the result. Also redact PII here.` },
  fresh:`<ul><li><strong>30+ hook events now</strong> — beyond <code>PreToolUse</code>/<code>PostToolUse</code>, current docs add <code>PostToolUseFailure</code>, <code>PermissionRequest</code>/<code>PermissionDenied</code>, <code>SubagentStart</code>/<code>SubagentStop</code>, <code>PreCompact</code>/<code>PostCompact</code>, <code>InstructionsLoaded</code>, <code>FileChanged</code>, <code>WorktreeCreate</code>, and more. <a href="https://code.claude.com/docs/en/hooks">docs</a></li><li><strong>A 4th <code>permissionDecision</code></strong> — <code>PreToolUse</code> can now return <code>defer</code> (alongside <code>allow</code>/<code>deny</code>/<code>ask</code>) and can rewrite the call's args via <code>updatedInput</code> — interception is no longer just block-or-allow. <a href="https://code.claude.com/docs/en/hooks">docs</a></li><li><strong>Five hook types</strong> — hooks can be <code>command</code>, <code>http</code>, <code>mcp_tool</code>, <code>prompt</code> (Claude-evaluated), or <code>agent</code> (subagent verification, experimental), broadening "run a script" to LLM- and agent-driven checks. <a href="https://code.claude.com/docs/en/hooks">docs</a></li><li><strong>SDK hooks are callbacks</strong> — the exam guide's hooks live in <code>.claude/settings.json</code>; in the Agent SDK they're callback functions passed via the <code>hooks</code> query option, not external scripts. <a href="https://code.claude.com/docs/en/agent-sdk/overview">docs</a></li></ul>`,
  quick:[
...
# subsequent subtasks are generated from what step 1 discovered, not fixed up front.` },
  fresh:`<ul><li><strong><code>effort</code> shrinks the need to decompose</strong> — the <code>effort</code> parameter (<code>xhigh</code> for Fable 5 / Opus 4.7+) lets newer models handle well-scoped tasks that previously had to be broken down manually. <a href="https://platform.claude.com/docs/en/build-with-claude/extended-thinking">docs</a></li><li><strong><code>Workflow</code> tool for scale</strong> — dynamic decomposition can now run across tens to hundreds of parallel agents via the <code>Workflow</code> tool, externalizing the planning loop. <a href="https://code.claude.com/docs/en/agent-sdk/overview">docs</a></li><li><strong>Tool-search defers schema loading</strong> — MCP tool-search loads tool schemas on demand by default, freeing context for the task content itself rather than upfront tool definitions. <a href="https://code.claude.com/docs/en/mcp">docs</a></li></ul>`,
  quick:[
...
claude -p "Fresh start. Summary of prior findings: <structured summary>. Re-investigate."` },
  fresh:`<ul><li><strong>Three SDK continuation modes</strong> — the exam guide's <code>--resume</code> maps onto three SDK modes: <code>continue</code> (most-recent session in the cwd, no ID needed), <code>resume</code> (by session ID), and <code>fork_session</code>/<code>forkSession</code> (branch without modifying the original). <a href="https://code.claude.com/docs/en/agent-sdk/sessions">docs</a></li><li><strong>Where session files live</strong> — sessions persist as <code>~/.claude/projects/&lt;encoded-cwd&gt;/*.jsonl</code> (override the root via <code>CLAUDE_CONFIG_DIR</code>); resuming on another machine means copying that file across. <a href="https://code.claude.com/docs/en/agent-sdk/sessions">docs</a></li><li><strong>In-memory-only sessions</strong> — in the TS SDK, <code>persistSession:false</code> keeps a session in memory only, so nothing is written to disk. <a href="https://code.claude.com/docs/en/agent-sdk/sessions">docs</a></li><li><strong>Session utilities</strong> — the SDK exposes <code>listSessions()</code>, <code>renameSession()</code>, <code>tagSession()</code>, and <code>getSessionMessages()</code> to manage and inspect saved sessions programmatically. <a href="https://code.claude.com/docs/en/agent-sdk/sessions">docs</a></li><li><strong>Stateful vs per-query</strong> — Python's <code>ClaudeSDKClient</code> tracks session IDs statefully across calls, whereas the TS SDK opts in per query with <code>continue:true</code>. <a href="https://code.claude.com/docs/en/agent-sdk/sessions">docs</a></li></ul>`,
  quick:[
